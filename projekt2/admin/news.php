<?php 
	...
	if (isset($_POST['_action_']) && $_POST['_action_'] == 'add_news') {
		...
		$result = @mysqli_query($MySQL, $query); var_dump($query); exit();
        ...
        if($_FILES['picture']['error'] == UPLOAD_ERR_OK && $_FILES['picture']['name'] != "") {
			...
			if ($ext == '.jpg' || $ext == '.png' || $ext == '.gif') { # test if format is picture
				...
				$_SESSION['message'] .= '<p>Uspjesno ste dodali fotografiju.</p>';
			}
        }
		
		
		$_SESSION['message'] .= '<p>Uspjesno ste dodali vijest!</p>';
	...
	if (isset($_POST['_action_']) && $_POST['_action_'] == 'edit_news') {
		...
		$_SESSION['message'] = '<p>Uspjesno ste izmjenili vijest!</p>';
	...
	if (isset($_GET['delete']) && $_GET['delete'] != '') {
		...
		$_SESSION['message'] = '<p>Uspjesno ste izbrisali vijest!</p>';
	...
	#Edit news
		...
		<form action="" id="news_form_edit" name="news_form_edit" method="POST" enctype="multipart/form-data">
			...
			<label for="description">Tekst *</label>
			<textarea id="description" name="description" placeholder="Tekst vijesti.." required>' . $row['description'] . '</textarea>
